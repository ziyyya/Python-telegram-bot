...
class MovieBot:
    ...
    async def handle_movie_search(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if len(movie_name) > 100:  # Basic spam protection
        ...
        try:
            ...
            await update.message.reply_text("❌ Group access error. Try later.")
...
async def cleanup_old_searches(bot_instance):
    """Clean expired searches every 5 minutes"""
    while True:
        try:
...
async def main():
    ...
    if PRIVATE_GROUP_ID == 0:
        logger.error("❌ PRIVATE_GROUP_ID environment variable required!")
        sys.exit(1)

    logger.info("🚀 Starting Movie Bot on Railway...")
